 ...
 */
class FanweAgent extends Agent
{
    ...
    public function sendContentSms($to, $content)
    {
        ...
        $params = [
            'content'  => $content,
            'mobile'        => $to,
            'is_adv' => 2,//0普通产品 1营销 2验证码
            'user_name'       => $this->user_name,
            'password'       => $this->password,
        ];

        $params_json = json_encode($params);
        $result_info = $sms->request("http://sms.fanwe.com/post",$params_json);

        $data = json_decode($result_info['body'], true);

        if ($data['status'] === 1) {
            $this->result['success'] = true;
        }
        $this->result['info'] = $data['msg'];
        $this->result['code'] = $data['status'];
    }
    ...
    public function voiceVerify($to, $code)
    {
        ...
        $this->result['info'] = 'Fanwe agent does not support voice verify';
        $this->result['code'] = '0';
    }
}
