...
@foreach ($products as $product)
                <li>
                    ...
                    <div class="listinfo2">
                        <div class="f_l f_redcolor {{ ($product->status == 1) ? 'btn-gray' : '' }}">{!! displayProductPrice($product) !!}</div>
    @if ($product->status == 1)
        ...
        @else
                        <a class="btn line btn-red f_r btn-gray" href="javascript:void(0)" role="button">查看详情</a>
        @endif
    @else
