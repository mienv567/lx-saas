...
class WeixinInfoController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(!empty($product_ids)){
            ...
            $weixin_template_id_shorts = array();
            foreach($wx_template as $item){
                $weixin_template_id_shorts[] = $item->weixin_template_id_short;
            }
            $wx_template_num = array_count_values($weixin_template_id_shorts);
            $data['template_count'] = count($wx_template_num);   //消息模板总数
        }
