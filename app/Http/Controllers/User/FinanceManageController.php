...
use App\Events\OrderFailDeal;
use App\Events\OrderSuccCloudPlatform;
use App\Events\OrderSuccCloudSystem;
use App\Events\OrderSuccServiceLlhb;
...
>>>>>>> .r650
     * @param  Request $request HTTP请求对象
     * @return \Illuminate\Http\Response   订单在线处理页面
     */
    public function payHandle(Request $request)
    {
        $user = $request->user();
        $orderId = $request->orderId;
        $orderPayMoney = $request->orderPayMoney; //订单应付金额
        $paymentTypeId = $request->paymentTypeId; //支付方式id
        $data = [
            'status' => 0, // 未支付
            'note' => '',
            'code' => '',
...
=======
        ...
        $payMoney = round($orderPayMoney - $accountPay, 2);
        //在线支付
...
>>>>>>> .r650
     ...
     * @return void
     */
    public static function orderMakeProduct($order)
    {
        if ($order->product_type == 1 || $order->product_type == 2) {
            ...
            if ($result) {
                ...
                if ($order->product_type == 1) {
                    //触发云系统订单成功事件
                    event(new OrderSuccCloudSystem($order));
                } elseif ($order->product_type == 2) {
                    // 触发云平台成功事件
                    event(new OrderSuccCloudPlatform($order));
                }
            } else {
                $order->order_status = 3;
                $order->save();
                //触发订单处理失败事件
                event(new OrderFailDeal($order));
            }
            ...
            if ($result) {
                ...
                $order->save();
                //触发流量红包订单成功事件
                event(new OrderSuccServiceLlhb($order));
            } else {
                $order->order_status = 3;
                $order->save();
                //触发订单处理失败事件
    ...
    public function orderDetail(Request $request)
    {
        ...
        $consumes = $order->consumes()->where('pay_type', 0)->get();
        $payments = $order->payments()->where('is_paid', 1)->get();
    ...
    public function orderCancel(Request $request)
    {
        ...
        try {
            ...
            Log::error($errmsg);
            DBLog::error($errmsg);
            return parent::responseJson(1002, '订单取消异常,请重新提交');
        }
    }
