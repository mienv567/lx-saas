...
//		  'ctl' => 'weixin',
//		  'act' => 'accept',
//		  'signature' => 'd8ac1cdc46b2df379b8006768055b7a76ad8d65e',
//		  'timestamp' => '1460692246',
//		  'nonce' => '978373443',
//		  'encrypt_type' => 'aes',
//		  'msg_signature' => 'd75bdef33e6251840304d2e98487d1190b5e4b65',
//		);
	...
	public function check_platform_access_token($platform_appid='',$platform_appsecret='',$platform_component_verify_ticket='',$platform_component_access_token=''){
		...
		if ($result)
		{
			...
			DB::query("update ".DB_PREFIX."weixin_conf set value='".$this->platform_component_access_token."' where name='platform_component_access_token' ");
			DB::query("update ".DB_PREFIX."weixin_conf set value='".$expire."' where name='platform_component_access_token_expire' ");
...
//			$GLOBALS['db']->query("update ".DB_PREFIX."weixin_conf set value='".$expire."' where name='platform_pre_auth_code_expire' ");
	...
	public function platform_api_query_auth($authorization_code,$type='',$user_id=''){
		...
		if ($result)
		{
			...
			$author_info['expires_in']=get_gmtime()+$author_info['expires_in'];
			...
			$id = DB::table(DB_PREFIX."weixin_account")->insertGetId($author_info);
	...
	public function check_platform_authorizer_token(){
		...
		if ($result)
		{
			...
			DB::table(DB_PREFIX."weixin_account")->where('authorizer_appid',$this->authorizer_appid)->update($data);
	...
	public function platform_get_authrizer_info(){
		...
		if ($result)
		{
			...
			//DB::table(DB_PREFIX."weixin_account")->insertGetId(array('authorizer_appid'=>$auth_info['authorizer_appid']));
			DB::table(DB_PREFIX."weixin_account")->where('authorizer_appid',$auth_info['authorizer_appid'])->update($auth_info);
		...
		if ($result)
		{
			...
			if (!$json || $json['errcode']!=0) {
				...
				if($json['errmsg']=='ok'){
					...
					DB::table(DB_PREFIX."weixin_account")->where('authorizer_appid',$this->authorizer_appid)->update($auth_info);
					return true;
 				}
			}
...
class WXBizMsgCrypt
{
	...
	public function decryptMsg($msgSignature, $timestamp = null, $nonce, $postData, &$msg)
	{
		...
		if ($ret != 0) {
 		...
 		$signature = $array[1];
		log_result(var_export(array('signature'=>$signature,'msgSignature'=>$msgSignature),true));
...
{
	$file = APP_ROOT_PATH."/storage/logs/notify_weixin.log";;
	$fp = fopen($file,"a");
	flock($fp, LOCK_EX) ;
